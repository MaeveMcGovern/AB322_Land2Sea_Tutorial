...
```{r}
## For the next operation we are working in the old sf/raster formats
# Change sf file format to sp file format
watershed_sp <- sf:::as_Spatial(watershed$geometry)
river_sp <- sf:::as_Spatial(river_shed$geometry)
station_sp <- sf:::as_Spatial(stations_projected$geometry)
```


```{r}
## Calculate the distance to larger water bodies

#Join pieces of polygons to speed up processing
river_sp2<-gUnion(river_sp, river_sp)

#Make an empty raster layer with the extent of the watersheds
r = raster(extent(watershed_sp),res=90)
...
d = gDistance(p, river_sp2, byid=TRUE)
...
distance<-rast(r)
crs(distance)  <- "EPSG:32633"

plot(distance)
plot(st_geometry(watershed), add=TRUE)
```

#### Calculate the distance to the sampling station
```{r}
...
## Because the water-f shapefile is not very good, we make two buffers around the river lines shapefile 
# for example to find potential erosion zones
...
#We repeat this for a 200 meter buffer
#Buffer 200
buffer200 <- st_buffer(riverline_shed, dist = 200)

v200 <- vect(buffer200)
r200 <- rast(v200,res=10)
z200 <- rasterize(v200, r200, "name")
names(z200)[1] <- "buff200"
```

#### Terrain properties from Raster files
```{r}
## Slope
...
stack<-c(veg_crop, ndvi_prj, dem_prj, slope_prj, distance_prj, buffer50_prj, buffer200_prj, station_distance)
...
watershed_df<-dplyr::select(watershed_df, watershed = ID, vegetation = i_vegetation, ndvi = i_NDVI_average2000_2014, elevation = dem_svalbard_center, slope, 
                            station_distance=Elva, river_distance = layer, river_buffer50 = buff50, river_buffer200 = buff200)
...
watershed_df<-mutate(watershed_df, river_buffer200 = ifelse(grepl ("elva", river_buffer200), "1", river_buffer200))
...
# Save the file
write.table(watershed_df, file = "Datafiles/spatial_data/watershed_dataframe.txt", append = FALSE, sep =  "\t", dec = ".",
            row.names = FALSE, col.names = TRUE)
```

#### Calculate % area of glaciers and watershed based on polygons
```{r}
...
#Change the percentage/km2 to nummeric format
...
#Save the file
write.table(watershed_info, file = "Datafiles/spatial_data/watershed_info.txt", append = FALSE, sep =  "\t", dec = ".",
            row.names = FALSE, col.names = TRUE)
```
