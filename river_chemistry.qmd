...
Water_Chem$Month <- factor(Water_Chem$Month,levels = c("May", "June", "August", "September"))
```
...
P1 <- Water_Chem %>% 
  dplyr::select(River, Month, DOC) %>% drop_na() %>% 
  ggplot(aes(x = Month, y = DOC, fill = Month)) + 
  geom_boxplot() +
  scale_fill_manual(values=c("cornflowerblue","tan","lightsalmon4", "blue")) +
...
P2 <- Water_Chem %>% 
  dplyr::select(River, Month, NO2_NO3, SiO2, PO4) %>% 
...
P3 <- Water_Chem %>% 
  dplyr::select(River, Month, TotHg_Dissolved, TotHg_Particulate) %>% 
...
P4 <- Water_Chem %>% 
  dplyr::select(River, Month, TotHg_g_DOC, TotHg_g_POC) %>% 
  pivot_longer(cols= -c(River, Month), names_to = "Mercury", values_to= 'Concentration') %>% 
  ggplot(aes(x = Month, y = Concentration, fill = Month)) + 
  geom_boxplot() +
  scale_fill_manual(values=c("cornflowerblue","tan","lightsalmon4", "blue")) +
